...
export async function getAllFiles(client: PluginClient<any, Readonly<IRemixApi>> & PluginApi<Readonly<IRemixApi>>): Promise<SolFile[]> {
    ...
    const resolveFolder: any = async (folder: Folder) => {
        const files = Object.keys(folder).map(async (path: string) => { 
            if (folder[path].isDirectory) {
                return (await Promise.all(await resolveFolder(await client.fileManager.getFolder(path))));
            } else {
                return {
                    path,
